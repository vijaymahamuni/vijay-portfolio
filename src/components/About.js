const About = () => {
  return (
    <div className=" bg-[#0a192f]  text-white">
      <div className="mx-auto w-9/12 pt-12">
        <div>
          <h1 className="text-3xl font-bold">About Me</h1>
          <div className="mt-6 text-lg ">
            <h1>I am Vijay,a 24-year-old Software Developer.</h1>
            <h1 className="mt-2 text-lg">
              I'm a passionate web developer with a strong focus on building
              robust and dynamic web applications.
            </h1>
            <h1 className="mt-2 text-lg">
              I take into consideration the user experience while writing
              reusable and efficient code. I passionately combine good design,
              technology, and innovation in all my projects.
            </h1>
            <h1 className="mt-2 text-lg">
              In addition to web development, I have a solid background in data
              analysis, particularly with WebFOCUS.
            </h1>
            <h1 className="mt-2 text-lg">
              Have completed my bachelor's degree in Computer Science and
              Engineering at GCET, Thanjavur.
            </h1>
          </div>
        </div>

        <div className="mt-12">
          <h1 className="text-3xl font-bold">My Skills</h1>
          <h1 className="mt-3 text-gray-400">
            Technologies I have worked with and I am familiar with.
          </h1>
          <h1 className="mt-8">Frontend Development</h1>

          <div className="mt-1 flex flex-wrap justify-between">
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg"
                className="w-12 ml-1"
                alt=""
              />{" "}
              <h1 className="text-black ml-4">JavaScript</h1>
            </div>
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original-wordmark.svg"
                className="w-12 ml-2"
                alt=""
              />
              <h1 className="text-black ml-6">React</h1>
            </div>
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg"
                className="w-12 ml-2"
                alt=""
              />{" "}
              <h1 className="text-black ml-6">HTML</h1>
            </div>
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-plain.svg"
                className="w-12 ml-2"
                alt=""
              />{" "}
              <h1 className="text-black ml-6">CSS</h1>
            </div>
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://www.vectorlogo.zone/logos/tailwindcss/tailwindcss-icon.svg"
                className="w-12 ml-1"
                alt=""
              />{" "}
              <h1 className="text-black ml-2">Tailwind CSS</h1>
            </div>
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://raw.githubusercontent.com/devicons/devicon/master/icons/redux/redux-original.svg"
                className="w-12 ml-2"
                alt=""
              />{" "}
              <h1 className="text-black ml-6">Redux</h1>
            </div>
          </div>

          <h1 className="mt-4">Backend Development</h1>

          <div className="mt-1 flex flex-wrap">
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original-wordmark.svg"
                className="w-12 ml-2"
                alt=""
              />{" "}
              <h1 className="text-black ml-6">NodeJS</h1>
            </div>
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://w7.pngwing.com/pngs/747/798/png-transparent-mysql-logo-mysql-database-web-development-computer-software-dolphin-marine-mammal-animals-text-thumbnail.png"
                className="w-12 ml-2"
                alt=""
              />{" "}
              <h1 className="text-black ml-6">MySQL</h1>
            </div>
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-plain.svg"
                className="w-12 ml-1"
                alt=""
              />{" "}
              <h1 className="text-black ml-3">Mongo DB</h1>
            </div>
          </div>
          <h1 className="mt-4">WebFOCUS Development</h1>

          <div className="mt-1 flex flex-wrap justify-between">
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://store-images.s-microsoft.com/image/apps.200.3ca565e9-ba93-4730-a4f7-0d4e3e9a9a63.2b6b3291-3d7d-4dc9-9197-ab9eb51c3529.b29f50f0-6f2f-425e-b3ec-e7f9bd4598e9"
                className="w-12 ml-1"
                alt=""
              />
              <h1 className="text-black ml-4">WebFOCUS</h1>
            </div>
          </div>
          <h1>Mobile Developement</h1>
          <div className="mt-1 flex flex-wrap justify-between">
            <div className="py-2 px-4 bg-gray-50 md:m-4 mx-2 mt-6 rounded-lg flex  items-center  cursor-pointer md:w-48 w-40  ">
              <img
                src="https://pagepro.co/blog/wp-content/uploads/2020/03/react-native-logo-884x1024.png"
                className="w-12 ml-1"
                alt=""
              />{" "}
              <h1 className="text-black ml-2">React Native </h1>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default About;
